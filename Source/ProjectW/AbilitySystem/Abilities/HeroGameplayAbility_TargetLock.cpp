...
void UHeroGameplayAbility_TargetLock::OnTargetLockTick(float DeltaTime)
{
	...
	const bool bShouldOverrideRotation =
		UWarriorFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), WarriorGameplayTags::Player_Status_Rolling) == false
		&& UWarriorFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), WarriorGameplayTags::Player_Status_Blocking) == false;
...
{
	if (IsValid(CurrentLockedActor) == false || AvailableActorsToLock.IsEmpty())
	{
		CancelTargetLockAbility();
	...
	{
		if (IsValid(AvailableActor) == false || AvailableActor == CurrentLockedActor)
			continue;
...
{
	if (IsValid(DrawnTargetLockWidget) == false || IsValid(CurrentLockedActor) == false)
	{
		CancelTargetLockAbility();
...
{
	if (IsValid(GetHeroControllerFromActorInfo()) == false)
		return;
